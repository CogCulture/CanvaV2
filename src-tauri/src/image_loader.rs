...
pub fn is_image_cached(path: String, state: axum::extract::State<std::sync::Arc<AppState>>) -> bool {
            ...
            let result: Result<(DynamicImage, HashMap<String, String>), String> =
                ...
                (|| match read_file_mapped(Path::new(&path_clone)) {
                    ...
                    Err(e) => {
                        ...
                        let bytes = if path_clone.starts_with("data:image/") && path_clone.contains(";base64,") {
                            if let Some(b64) = path_clone.split(";base64,").nth(1) {
                                use base64::Engine;
                            ...
                            fs::read(&path_clone).map_err(|io_err| {
                        ...
                        let is_data_url = path_clone.starts_with("data:image/") && path_clone.contains(";base64,");

                        let ext_check_path = if is_data_url {
                            "virtual_canvas_image.png".to_string()
                        } else {
                            path_clone.clone()
                        };
                        ...
                        let exif = exif_processing::read_exif_data(&ext_check_path, &bytes);

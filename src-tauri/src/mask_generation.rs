...
fn generate_sub_mask_bitmap(
    ...
    match sub_mask.mask_type.as_str() {
        ...
        "rectangle" => Some(generate_rectangle_bitmap(
...
pub fn resolve_warped_image_for_masks(
    ...
    if masks.iter().any(|m| m.requires_warped_image()) {
        ...
        get_cached_full_warped_image(state.clone(), adjustments).ok()
...
#[serde(rename_all = "camelCase")]
struct RectangleMaskParameters {
    start_x: f64,
    start_y: f64,
    end_x: f64,
    end_y: f64,
...
fn generate_rectangle_bitmap(
    ...
    let params: RectangleMaskParameters =
        serde_json::from_value(params_value.clone()).unwrap_or_default();
    println!("RECTANGLE PARAMS (raw): {:?}", params_value);
    println!("RECTANGLE PARAMS (parsed): start_x={}, start_y={}, end_x={}, end_y={}", params.start_x, params.start_y, params.end_x, params.end_y);
    let mut mask = GrayImage::new(width, height);

    let start_x = (params.start_x as f32 * scale - crop_offset.0) as i32;
    let end_x = (params.end_x as f32 * scale - crop_offset.0) as i32;
    let start_y = (params.start_y as f32 * scale - crop_offset.1) as i32;
    let end_y = (params.end_y as f32 * scale - crop_offset.1) as i32;
    ...
    let rect_width = (max_x - min_x) as f32;
    let rect_height = (max_y - min_y) as f32;
    let feather_pixels = (params.feather * rect_width.min(rect_height) * 0.5).max(1.0);
    ...
    for y in min_y..=max_y {
        ...
        for x in min_x..=max_x {
            ...
            let dist_x = ((x - min_x).min(max_x - x)) as f32;
            let dist_y = ((y - min_y).min(max_y - y)) as f32;
            
            let dist_to_edge = dist_x.min(dist_y);
            
            let intensity = if dist_to_edge >= feather_pixels {
                1.0
            } else {
                dist_to_edge / feather_pixels
            };

            let pixel_val = (intensity * 255.0) as u8;
            mask.put_pixel(x as u32, y as u32, Luma([pixel_val]));
        }
    }
    mask
}

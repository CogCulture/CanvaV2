...
pub async fn generate_ai_foreground_mask(
    ...
    let warped_image = get_cached_full_warped_image(state.clone(), &js_adjustments)?;
...
pub async fn check_ai_connector_status(app_handle: crate::DummyAppHandle) {
...
pub async fn invoke_generative_replace_with_mask_def(
    ...
    let (base_image, _) = get_full_image_for_processing(state.clone())?;
